        ...
        options: {
            ...
            content: function () {
                ...
                var $td = $(this),
                    ...
                    // cellCustomTooltip 이벤트를 발생
                    callback = grid.options['cellCustomTooltip'], tpText;
                    if (callback) {
                        tpText = callback.call(grid, { $td: $td, $grid: $grid, grid: grid, obj: obj })
                    }
                    if (tpText) {
                        return tpText;
                    }
            ...
            close: function (evt, ui) {
                //debugger;
                //Element가 계속 누적되는 현상이 발생되어, close시점에서 해당 element를 삭제해준다.
                $(".ui-helper-hidden-accessible").children()[0].remove();
                //ui.tooltip.children()[0].remove();
            }
            // <-- [Edit] kim changa 2019/09/19
        }
    };
...
(function($) {
    ...
    (function() {
        ...
        pq[s] = function(ele, val) {
            ...
            if (rtl) {
                if (type == "w") ele[s] = ele.scrollWidth - ele.clientWidth - val;
                else if (type == "g") ele[s] = -1 * val;
                else ele[s] = val
            } else ele[s] = val
        }
    })()
...
(function($) {
    ...
    $.extend(pqPager.prototype, {
        ...
        getPageOf: function() {
            var options = this.options;
            return "<td><span>" + (options.strPage || "").replace("{0}", "<input type='text' value='" + options.curPage + "' tabindex='0' class='pq-page-current ui-corner-all' />").replace("{1}", "<span class='pq-page-total'>" + options.totalPages + "</span>") + "</span></td>"
        },
...
(function($) {
    ...
    fni._create = function() {
        ...
        that._initTypeColumns();
        element.on("scroll" + that.eventNamespace, function() {
            this.scrollLeft = 0;
            this.scrollTop = 0
        }).on("mousedown" + that.eventNamespace, that._mouseDown.bind(that));
    ...
    fn.getValueFromDataType = function(val, dataType, validation) {
        ...
        // --> [Edit] 수정자:kim changa 일시:2019/01/16
        // 수정내용>  changha
        // sort 시에 값이 dataType별로 바뀌는 현상으로 소트사용인 경우 무시
        // addRow 함수 추가시에 문제
        // start 수정
        return val
        // start 끝
/*
    ...
    fn._onMouseDownCont = function(evt) {
        ...
        if (!pdata || !pdata.length) {
            ...
            // --> [Edit] 수정자:이노한 일시:2018/12/05
            // 수정내용> 그리드 데이터가 없을때 클릭시 포커스 이동문제 수정
            //cont.focus()
            // <-- [Edit] 이노한 2018/12/05
        }
    };
    ...
    fn._onCellMouseLeave = function(that) {
        return function(evt) {
            if (that.evtBelongs(evt)) {
                // --> [Edit] 수정자: kim changa 일시:2019/09/19
                // 수정내용> cellMouseLeave 이벤트를 발생하기 위해서 변경
                /* 원본소스
                var $td = $(this);
                ...
                */
                ...
                if (that._trigger("cellMouseLeave", evt, objP) === false) {
                    return
                }
                if (that.options.hoverMode == "cell") {
                    that.unHighlightCell($td)
    ...
    fn._onClickCell = function(evt) {
        ...
        // --> [Edit] lee nohan 2019/09/09
        // CellClick에서 화면을 이동시키는 경우 (ex. BisPex거래소 Assets - Balance 화면에서 Trade버튼을 클릭시, Trade화면으로 이동되는 경우)
        // 화면 이동처리가 일어나면서, 해당 객체가 사라져서, 밑에 that.isEditable함수 내에서 오류가 발생.
        // options이 undefined인 경우 return 시키도록 변경함
        if (that.options == undefined) return;
        // <-- [Edit] lee nohan 2019/09/09

        if (EM.clicksToEdit == 1 && that.isEditable(objP)) {
            // --> [Edit] lee nohan 2019/08/16
            // 체크박스일때는 editable false로 앞단에서 처리함
            //if (objP.column.type && objP.column.type != "checkbox")
            // <-- [Edit] lee nohan 2019/08/16
            that.editCell(objP)
        }
    ...
    fn.generateLoading = function() {
        // --> [Edit] 수정자:kim changa 일시:2019/01/16
        // 수정내용> loading 기능 옵션처리 ( loading : true 인 경우만 허용)
    ...
    fn._refreshPager = function() {
        ...
        if (paging) {
            ...
            if (options.showBottom && hi5app_name != 'MTS') {
                that.$bottom.css("display", "")
            } else {
                that.$bottom.css("display", "none")
            }
    ...
    fn.refreshCell = function(ui) {
        ...
        if (this.iRenderB.refreshCell(rip, ci, obj.rowData, obj.column)) {
            // --> [Edit] lee nohan 2019/08/16
            // 실시간 들어올때 포커스가 가면서, 화면스크롤이 움직이는 현상 수정
    ...
    fn.refreshRow = function(_obj) {
        ...
        // --> [Edit] 수정자:kim changa 일시:2019/01/16
        // 수정내용> changha fn.focus() 함수에서 Scroll 이동되는 현상건
        // 원본
        ...
        // end
        // refreshRow trigger시 setTimer로 focus 이동시키는 현상으로 하단 막음.
        // 상단 _obj.focus 아래로 이동시킴.
        //that._trigger("refreshRow", null, {
        //    rowData: rowData,
        //    rowIndx: ri,
        //    rowIndxPage: rip
    ...
    cKeyNav.prototype = {
        ...
        incrIndx: function(rowIndxPage, colIndx, incr) {
            ...
            do {
                ...
                column = CM[colIndx];
                // --> [Edit] 수정자:kim changa 일시:2019/01/16
                // 수정내용> [LNH]20181211 처음과 마지막 컬럼이 hidden일 경우 무한루프에 빠지는 현상이 있어서 break; 추가
                if (colIndx == CMLength - 1 || colIndx == 0) break;
                // <-- [Edit] kim changa 2019/01/16
...
(function($) {
    ...
    cGenerateView.prototype = {
        ...
        // 수정내용> 컬럼리사이즈 getAllColsDiff 함수추가
        getAllColsDiff: function () {
            var that = this.that,
                CM = that.colModel,
                CMLength = CM.length,
                dims = this.dims,
                wdAllCols = that.calcWidthCols(-1, CMLength, true),
                sbWidth = 0;//this.getSBWd(),
                wdCont = dims.wdCenter - sbWidth;
            return { wdAllCols: wdAllCols, wdCont: wdCont, diff: wdAllCols - wdCont };
        },
        ...
        // --> [Edit] 수정자:kim changa 일시:2019/10/24
        // 수정내용> 컬럼리사이즈 getAllColsDiff 함수추가
                //px+% 셋팅 시에만 빌더에서 xml만들때 _auto_resize를 true 로 셋팅해서 만들어준다.
                //이때 _auto_resize가 true이면 그리드 전체 넓이에서, 스크롤영역 넓이를 빼줄때, 스크롤영역 넓이를 0으로 셋팅해준다....
                //이유는 좀더 찾아봐야할듯하다... 일단 원본소스로 변경..
                //sbWidth = that.options._auto_resize ? 0 : this.getSBWd(),
                // 원본소스
                //sbWidth = this.getSBWd(),
        // <-- [Edit] kim changa 2019/10/24
            //wdCont = dims.wdCenter - sbWidth;
           
            // --> [Edit] 2020/04/28 홍주형 그리드 컬럼 autofit 계산 수정
            // 조건문 or 조건식 반대로 변경(xml로 잡혀있는 그리드 사이즈 || 실제 그리드 사이즈) -> 실제그리드 사이즈가 없을때 xml사이즈 사용
            // this.$cright[0].clientWidth - this.getSBWd() -> this.$cright[0].clientWidth 
            // 실제 그리드 콘트영역이 스크롤 제외인데 getSBWd 이함수는 스크롤 너비를 가져오는 함수를 사용중이기에 제외
            //wdCont  = dims.wdContClient || this.$cright[0].clientWidth - this.getSBWd() ; // Scroll 영역 제외
            wdCont  =  this.$cright[0].clientWidth  || dims.wdContClient ; 
            if(typeof(wdCont) !== 'number' || wdCont == 0){
                return;
                //wdCont = parseInt(wdCont);
                //if(isNaN(wdCont)) wdCont = 0;
            }
            // <-- [Edit] 2020/04/28 홍주형 그리드 컬럼 autofit 계산 수정
            // --> [Edit] 2020/04/28 홍주형 그리드사이즈가 변경할때마다 이벤트를 타야하는데 실제 사이즈가 커질때만 타게되어있어서 전체로 변경
            //if (wdAllCols < wdCont) {
            // <-- [Edit] 2020/04/28 홍주형
                ...
                if(this.that.options.cellsizeunit != "px"){
        ...
        refreshColumnWidths: function(ui) {
            ...
            // 수정내용> options.lastautofit = ture  인경우 마지막 컬럼을 기능.
            var preColumn, totColWidth = numberCellWidth;  // 변수추가
            ...
            // --> [Edit] 수정자:kim changa 일시:2019/10/24
            // 수정내용> 컬럼리사이즈 컬럼사이즈 여분이 있는경우 autofit 적용한다.
            if (ui.correction) {
                if (flexWidth === false && ui.refreshWidth !== false) {
                    if (!autoFit) {
                        var obj = this.getAllColsDiff();
                        if (obj.diff != 0) {
                            if ( o.debug )
                                console.log("refreshColumnWidths", obj);
                            //return { wdAllCols: wdAllCols, wdCont: wdCont, diff: wdAllCols - wdCont };
                            autoFit = true;
                        }
                    }
            ...
            if (flexWidth === false && ui.refreshWidth !== false) {
                if (autoFit) {
                    ...
                    // 마지막 컬럼 사이즈 확대( 그리드 넓이 > 전체컬럼사이즈 합)
                    if (preColumn && contWd > totColWidth) {
                        var dif = contWd - totColWidth;
                        //console.log("--------", dif, this.dims);
                        preColumn._width = preColumn._width + dif;
                    }
                }
            }
            // <-- [Edit] kim changa 2019/10/31
...
(function($) {
    ...
    _pq.cHeader.prototype = {
        ...
        createHeaderCell: function(row, col, column, attr, cls, style) {
            var that = this.that,
                o = that.options,
// --> [Edit] 수정자:kim changa 일시:2019/02/03
// 수정내용> sortable = false 인경우 헤더에 소트 아이콘 비표시 기능
                SSS = column.sortable === false ? "" : this.getSortSpaceSpans(o.sortModel, row, col),
               // SSS = this.getSortSpaceSpans(o.sortModel),
// <-- [Edit] kim changa 2019/02/03
            ...
            // --> [Edit] 수정자:kim changa 일시:2019/01/16
            // 수정내용> // changha title값이 "" 이면 헤더영역에 checkbox가 표시안되는 현상
            //title = title != null ? title : type == "checkbox" && column.cb.header ? "<input type='checkbox'/>" : column.dataIndx;
            // start 수정
            // --> [Edit]  LNH 2019/01/21 type => column.type 으로 변경
            //if (column.cb && column.cb.header && type === "checkbox") {
            if (column.cb && column.cb.header && column.type === "checkbox") {
                // <-- [Edit]  LNH 2019/01/21
            ...
            // --> [Edit] 수정자:kim changa 일시:2019/01/16
            // 수정내용> // column.clsHead 객체존재 유무 체크, cls 는 Header 부분에서는 제거를 하다.
            //if (ccls) cls.push(ccls);
            if (column.clsHead) cls.push(column.clsHead);
            // <-- [Edit] kim changa 2019/01/16
            ...
            // --> [Edit] 수정자:lnh 일시:2019/01/16
            // 수정내용> // column.Hstyle 헤더 스타일이 있을경우 div에 스타일을 작성해준다.(refresh 할때 기존에 정렬맞춰놓은 스타일이 사라지는 현상 수정)
            var strHStyle = "";
            if(column.Hstyle){
                var objKeys = Object.keys(column.Hstyle);
                for(var i = 0; i < objKeys.length ; i++){
                    strHStyle += objKeys[i];
                    strHStyle += ":" + column.Hstyle[objKeys[i]] + ";";
                }
            }
            // <-- [Edit] lnh 2019/01/16
            ...
            // --> [Edit] 수정자:kim changa 일시:2019/09/19
            // 수정내용> 그리드 커스텀 툴팁	
            var tp_cls = ["pq-title-span"];
            if (column.data_h_tp_key) {
                tp_cls.push("pq-title-span-tooltp");
            }
            return ["<div ", colIndxDD, " ", rowIndxDD, " ", tabindex, " ", attr, " ", " class='", cls.join(" "), "' style='", style.join(""), "' >", "<div class='pq-td-div' style='" + strHStyle + "'>", collapsedStr, "<span class='", tp_cls.join(" "), "'>", title, "</span>", SSS, "</div>", hasMenuH ? "<span class='pq-menu-icon'></span>" : "", "</div>"].join("")
        ...
        getSortSpaceSpans: function (SM, row, col) {
            var pq_space = SM.space ? " pq-space" : "";

            // --> 20190829 lee nohan
            //sort시 normal이 표시되었다가, up, down 화살표가 그려지는 현상을 방지하기위해 로직 변경

            // --> [Edit] 수정자:kim changa :2019/01/16
            //pq_space += (SM.on ? " pq-header-sort-normal" : "");
            //pq_space += (SM.on ? " fas fa-sort" : "");
            // <-- [Edit] kim changa 2019/01/16            
            //return ["<span class='pq-col-sort-icon", pq_space, "'></span>", SM.number ? "<span class='pq-col-sort-count" + pq_space + "'></span>" : ""].join("")
            //return ["<i class='pq-col-sort-icon", pq_space, "'></i>", SM.number ? "<span class='pq-col-sort-count" + pq_space + "'></span>" : ""].join("")
            ...
            if(SM.on){
                //debugger;
                var sortDataIndx;
                if(SM.sorter.length > 0){
                    sortDataIndx = SM.sorter[0].dataIndx;
                }
                //debugger;
                //sort click시 sortDataIndx 값이랑 현재 그리려는 헤더 dataIndx 랑 비교해서 맞으면 up,down 표시
                if(sortDataIndx == that.headerCells[0][col].dataIndx){
                    var dir = SM.sorter[0].dir;
                    var cls2 = BS.on ? " glyphicon glyphicon-arrow-" + dir : dir == "up" ? " fa fa-sort-up up_txt" : " fa fa-sort-down low_txt";
                    pq_space += cls2;
                }else{
                    pq_space += " fa fa-sort";
                }
            }
            return ["<i class='pq-col-sort-icon", pq_space, "' style='width:14px;height:13px'></i>"].join("")
            // 원본
            //return ["<span class='pq-col-sort-icon", pq_space, "'></span>", SM.number ? "<span class='pq-col-sort-count" + pq_space + "'></span>" : ""].join("")
        },
...
// --> [Edit] 수정자:kim changa 일시:2019/01/17
// 수정내용> dataUF Undefined 되어서 [] 초기화 한다
                dataUF = DM.dataUF || [],
// <-- [Edit] kim changa 2019/01/17
        ...
        onWindowResize: function(evt, ui) {
            ...
            // --> [Edit] 수정자:kim changa 일시:2019/01/16
            // 수정내용> soft: true값을 false 변경
            ui = ui || {};
            if (!ui.self) ui.self = this;
            ui.ISZOOM = cRefresh.ISZOOM;
        ...
        refresh: function(ui) {
            ...
            // --> [Edit] 수정자:kim changa 일시:2019/10/24
            // 수정내용> 그리드 사이즈 변경전
            //if (o.debug && ui.windowResize) {
            var _auto_resize = false, preDims, correction = false;
            if (o._auto_resize && ui.windowResize) {
                _auto_resize = true;
                // 사이즈 정보복사
                preDims = JSON.parse(JSON.stringify(dims));
                if (o.debug)
                    console.log("refresh.._auto_resize", preDims)
            }
            self.refreshGridWidthAndHeight();
            //  refreshFull 이벤트 추가
            if (_auto_resize ) {
                correction = that._trigger("autoResizeBefore", null, {
                    preDims: preDims,
                    that: that,
                    dims: dims
                })
            ...
            // --> [Edit] 수정자:kim changa 일시:2019/10/24
            // 수정내용> windowResize 키 추가(cRenderBody)
            that.iRenderB.init({
                correction: correction,  // 추가
                windowResize: ui.windowResize || false,   // 추가
                header: header,
                soft: ui.soft,
                source: ui.source
            });
...
(function($) {
            ...
            that.one("refreshDone", function() {
                ...
                setTimeout(function() {
                    if (that.element) {
                        ...
                        // 인자추가
                        that._trigger("complete", "", o.dataModel.data)
                    }
                }, 0)
            })
        });
        // 생성시에 rendering 방지 options 속성에 _initRefreshSkip = true 인 경우만 허용
        // start 수정
        //  if (o._initRefresh) {
        that.refreshDataAndView({
            header: true
    ...
    fn.focus = function(_ui) {
        ...
        if (realFocus) {
            ...
            if ($td && (td = $td[0]) && $td.hasClass("pq-grid-cell") && !td.edited) {
                ...
                // --> [Edit] 수정자:kim changa 일시:2019/10/24
                // 수정내용> o.nofocus 옵션값에 땨라서 포커스 이동처리
    ...
    fn._refreshHeaderSortIcons = function() {
        // --> [Edit] 수정자:kim changa 일시:2019/01/16
        // 수정내용> this.iHeader.refreshHeaderSortIcons() 함수 제거( 이유?)
...
// --> [Edit] 수정자:kim changa 일시:2019/10/24
// 수정내용> V6.0.0 cls = _cls + " " + range.type 에서 6.2.4 버전은 cls = _cls 변경..options.layer 옵션으로 표시유무변경
                cls = that.options.layer ? _cls  :_cls + " " + range.type,
// <-- [Edit] kim changa 2019/10/24
            ...
            // --> [Edit] 수정자:kim changa 일시:2019/10/24
            // 수정내용> left,top 값이 -1 이면 0 으로 보정
            //left = tmp[0] - 1;
            //top = tmp[1] - 1;
            left = tmp[0] > 0 ? tmp[0] - 1 : 0;
            top  = tmp[1] > 0 ? tmp[1] - 1 : 0;
            // <-- [Edit] kim changa 2019/10/24
...
(function($) {
    ...
    cMouseSelection.prototype = $.extend({
        onCellMouseDown: function(evt, ui) {
            ...
            if (type == "cell") {
                ...
                if (evt.shiftKey && mode !== "single" && last && last.firstR != null) {
                    ...
                    if (colIndx == null) {
                        ui.colIndx = that.getFirstVisibleCI();
                        var len = that.getColModel().length;
                        iSel.resizeOrReplace({
                            r1: rowIndx,
                            c1: ui.colIndx,
                            rc: 1,
        ...
        onRefresh: function() {
            var that = this.that;
            // --> [Edit] 수정자:kim changa 일시:2019/01/16
            // 수정내용> 그리드 포커스시 강제로 이동되는 현상 수정(이노한)
...
(function($) {
    ...
    cFilterData.prototype = {
        ...
        hideRows: function(arrS, data, FMmode) {
            ...
            for (; i < len; i++) {
                rowData = data[i];
                rowData.pq_hidden = !this.isMatchRow(rowData, arrS, FMmode)
            }
        },
        filterLocalData: function(objP) {
            ...
            if (FM.hideRows) {
                this.hideRows(arrS, data1, FMmode)
            } else {
                if (apply) {
                    if (data2.length) {
                        filtered = true;
    ...
    cContext.prototype = {
        ...
        showMenu: function(evt, ui) {
            ...
            if (items && items.length) {
                ...
                $menu.css({
                    position: "absolute",
                    left: evt.pageX - (self.rtl ? $menu.width() : 0),
                    top: evt.pageY
                });
    ...
    cFillHandle.prototype = {
        ...
        onStop: function() {
            ...
            if (sel1.r1 != sel2.r1 || sel1.c1 != sel2.c1 || sel1.r2 != sel2.r2 || sel1.c2 != sel2.c2) {
                ...
                // autofill값이 true 인 경우만 허용하도록 수정
                if (autofill) {
                    // 원본
        ...
        getXlsHeader: function(hc, hcLen, mc) {
            ...
            for (var i = 0; i < hcLen; i++) {
                ...
                for (var ci = 0, lenj = row.length; ci < lenj; ci++) {
                    ...
                    // 수정내용> 엑셀내보내기 기능 ( <BR>요소를 "\r\n" 으로 변경)
                    cells.push({
                        //value: title(원본),
                        value: title.replace(/<br\s*[\/]?>/gi, "\r\n"),
                        align: "center",
                        valign: "center",
                        bgColor: "#eeeeee"
                    })
        ...
        getXlsBody: function(CM, CMLen, data, dataLen, remotePage, offset, iMerge, render, iGV, shiftR) {
            ...
            for (i = 0; i < dataLen; i++) {
                ...
                for (j = 0; j < CMLen; j++) {
                    ...
                    // 수정내용> 엑셀내보내기 기능
                    var _format = rowData._format;
                    if (_format && _format[di]) {
                        cell._format = _format[di];
    ...
    var pqEx = pq.excel = {
        ...
        getBody: function(rows) {
            ...
            for (i = 0; i < rowsLen; i++) {
                ...
                // --> [Edit] 수정자:kim changa 일시:2019/05/28
                // 수정내용> 엑셀내보내기 기능
                //  cell._format 포맷정보 ( 문자, 마스킹정보 커스커 마이즈)
                //  var _fomrat 변수 추가
                var _format;
                ...
                for (j = 0; j < cellsLen; j++) {
                    ...
                    if (value == null) {
                        v = "<v></v>"
                    } else if (value == parseFloat(value) && (value + "").match(/^[0-9,e,\-,\.]*$/gi)) {
                        v = "<v>" + value + "</v>"
            ...
            if (editorType == "text" || editorType == "textarea") {
                ...
                if (editorType == "textarea") {
                    var $ed = $editor.clone().appendTo($grid),
                        ed = $ed[0];
                    $ed.css({
                        // --> [Edit] 수정자:kim changa 일시:2019/10/29
                        // 수정내용> 18로 고정된 부분을 
                        height: $td.height() > 18  ? $td.height(): 18 ,  // 원본 18로 된 부분을 $td.height() 값으로 변경
                        // <-- [Edit] kim changa 2019/10/29
                        width: width,
                        position: "absolute",
                        left: 0,
                        top: 0,
...
(function($) {
    ...
    $(function() {
        ...
        cVirtual.setSBDim();
        // --> [Edit] 수정자:kim changa 일시:2019/10/24
        // 수정내용> 한번만 세로스크롤 폭값을 취득한다(17)
        //원본( 브라우저 사이즈 변동시 마다 사이즈 변동을 막는다.
...
(function($) {
    ...
    pq.cRender.prototype = $.extend({}, {
        ...
        getRowClsStyleAttr: function(rip) {
            ...
            // --> [Edit] 수정자:kim changa 일시:2019/01/16
            // 수정내용> 홀짝위치 조정(changha)
            /* 원본
            o.stripeRows && this.stripeArr[rip] && cls.push("pq-striped");
            */
            o.stripeRows && this.stripeArr[rip] && cls.push("grid_body_bg_odd");
            o.stripeRows && !this.stripeArr[rip] && cls.push("grid_body_bg_even");
            // <-- [Edit] kim changa 2019/01/16
            ...
            if (finalV >= 0) {
                for (; ci >= initH; ci--) {
                    ...
                    if (!column.hidden && (column.width + "").indexOf("%") == -1) {
                        // --> [Edit] 수정자:kim changa 일시:2019/10/24
                        // 수정내용> 헤더인 경우 column.width 사용( 헤더 더블클릭시에 현재사이즈로 기준으로 한다)
                        // 원본
                        //wd = takeColumnWidths ? column.width : column._minWidth;
                        wd = takeColumnWidths ? column.width : column.width;
                        // <-- [Edit] kim changa 2019/10/24
                        if (wd) {
...
(function($) {
    ...
    pq.cRenderBody.prototype = $.extend({}, new $.paramquery.cGenerateView, new pq.cRender, {
        ...
        init: function(obj) {
            ...
            // --> [Edit] 수정자:kim changa 일시:2019/10/24
            // 수정내용> refreshColumnWidths() obj 겍체인자 추가
            self.refreshColumnWidths(obj);
            // <-- [Edit] kim changa 2019/10/24
        ...
        setPanes: function() {
            ...
            // 수정내용> 윤성욱 FrozenCol시 왼쪽 그리드 하단 부 짤림 현상 방지 하기위해
            //$cl.height('100%');
            //console.log("setPanes---", clientHeight, $cl[0].offsetHeight, clientHeight -  $cl[0].offsetHeight);
            // 원본소스
    ...
    cRenderHead.prototype = $.extend({}, pq.cRenderHS, new _pq.cHeader, new _pq.cHeaderSearch, {
        ...
        onRefreshTimer: function(self, initHChanged) {
            return function() {
                ...
                // --> [Edit] 수정자:kim changa 일시:2019/01/16
                // 수정내용> refreshHeaderSortIcons() 호출제거
                // 원본 self.refreshHeaderSortIcons();
